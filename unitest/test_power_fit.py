...
LastEditTime: 2024-04-28 13:45:36
...
def fit_power():
    ...
    for i in range(1, 13):
        ...
        distance = (
            df.iloc[0:1, i * 4 - 2].astype("float32").repeat(len(delta_phi)).to_numpy()
        )
    ...
    print(list(poly._combinations(2, 1, 3, interaction_only=False, include_bias=True)))
...
def fit_power_mlp():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=1500, eta_min=1e-5
    )
...
def fit_power_interp():
    ...
    fig, ax = plt.subplots(1, 1, figsize=(2.4/1.5, 3.6/1.5))
    X_data = X_data.reshape([len(delta_phases), -1, 2])
    Y_data = Y_data.reshape([len(delta_phases), -1])
    xs = np.linspace(0, 2, 100)
    # cmap = plt.cm.turbo
    # norm = matplotlib.colors.Normalize(vmin=7, vmax=25)

    distances = np.array(distances)[:, 0]
    print(distances)

    # color = cmap(norm(distances))[::-1]
    color = ["#A21F34", "#D77F80", "#C5A6B0", "#A4B5D4", "#9FD4D9", 
             "#4C8BA7", "#0281CC", "#14587C", "#5725C0", "#611B59", "#611B59"]
    for i, (x, y) in enumerate(zip(X_data, Y_data)):
        # ax.scatter(x[:, 0],  y, c="b", label="simulated", marker="o")
        distance = np.zeros_like(xs) + distances[i]
        x = np.stack([xs, distance], -1)
        y_pred = interp(x)
        ax.plot(xs, y_pred, label="predicted", c=color[i])

        ax.set_xlabel("delta_phi (rad.)")
        ax.set_ylabel("power (mW)")
        ax.set_title(f"distance: {distances[i]} um")
        ax.set_xlim([0, np.pi / 2 + 0.5])
        ax.set_ylim([0, 7.5])
        # axes[i].set_xlim([0, np.pi/2+2.5])
        # axes[i].set_ylim([0, 9.5])
        # ax.legend(loc='upper left')
    plt.tight_layout()
    plt.savefig("./unitest/figs/power_interp_merge.png", dpi=300)
