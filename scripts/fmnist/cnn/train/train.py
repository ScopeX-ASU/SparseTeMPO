...
def task_launcher(args):
    lr, density, w_bit, in_bit, death_mode, growth_mode, init_mode, conv_block, id, gpu_id = args
    pres = [
        f"export CUDA_VISIBLE_DEVICES={gpu_id};",
        "python3", script, config_file]
    with open(
        os.path.join(
            root, f"{model}_{dataset}_lr-{lr:.3f}_wb-{w_bit}_ib-{in_bit}_dm-{death_mode}_gm-{growth_mode}_im-{init_mode}_cb-{conv_block}_den-{density}_run-{id}.log"
        ),
        "w",
    ) as wfid:
        exp = [
...
if __name__ == "__main__":
    ...
    tasks = [
        # (0.002, 0.5, 8, 8, "magnitude", "gradient", "uniform", [2, 2, 4, 4], 1, 1),
